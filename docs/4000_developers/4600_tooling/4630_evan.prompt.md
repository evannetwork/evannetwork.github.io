...
Don't let the name fool you, this is not just for working with blockchain contracts, the `evan` command line tool can be used to interact with all components of `evan.network`, i.e. also the [IPFS](/docs/developers/concepts/ipfs.html) file stores, the [ENS](/docs/developers/concepts/ens.html) etc.
...
So once you have [created a profile](/docs/first_steps/create-identity.html), you will need to copy your account ID, your private key and profile encryption key from the [dashboard](/docs/first_steps/dashboard.html), and put them into your `$HOME/.evan.json`.
...
Now that everything is installed and configured, let's take our ['Hello World' contract](/docs/developers/hello-world.html) and deploy it with the evan.network toolchain instead of `truffle`.
