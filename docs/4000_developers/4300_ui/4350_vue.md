...
When you want to use the `getRuntime` function with the root component itself, you can use the `loggedin` event, to use the runtime when the user has logged in. So just move your runtime logic from the `created` function, to another function.
