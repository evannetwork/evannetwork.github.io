...
```
Where the `vcStatus` holds either the value `active` or `revoked`. The `status` field in this examples does not describe the status of the VC but the status of the request. `"status": "success"` means, that the request against the server was successful.
