...
- `credentialStatus`
  - URL of the evan.network revokation service, allowing status checks about the VC. Only available for on-chain documents (see chapter *VC types in evan.network*).
- `proof`
...
Off-chain VCs are locally created VCs. They come with a valid proof, however, are not stored on the chain and thus cannot be recovered once lost. The evan.network revokation service does not support off-chain VCs, and encryption and sharing mechanisms have to be implemented manually.
...
On-chain VCs can be revoked using the evan.network revokation service and support data encryption and different stages of visibility, which are:
...
Certain credentials might become invalid over time. In this case, the VC for this credential must be revokable to stop the subject from continuing using the now invalid credential.  
