...
Off-chain VCs are locally created VCs. They come with a valid proof, however, are not stored on the chain and thus cannot be recovered once lost. The evan.network revocation service does not support off-chain VCs. Encryption and sharing mechanisms are supported but have to be implemented manually.
