...
As discussed in the previous section, although VCs stored on the even.network chain can be resolved, on-chain VCs also consist of a revocation status. A revocation flag is used to indicate that an on chain VC is no longer valid although the initial `validTo` field provided may still say the VC is valid. The URL to the revocation status of a VC is presented in the `credentialStatus.id` of the VC. The status end point returns a JSON object consisting of two key:value pair. Example of queried status of VC is as follows:
...
```
Where the `vcStatus` holds either the value `active` or `revoked` stating that a Vc is not revoked and revoked respectively. However, if one tries to request status of a non-existing or off chain VC, the endpoint will throw an error. on the other hand, the `status` key indicates if the request was successful or not.
