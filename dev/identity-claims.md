...
This function is provided by the Identity Service. In general, different proofs of identity, so-called verifications, can be assigned to an account. 
...
It is also not possible to trace which verifications an account has. Proofs are only selectively released by the respective account upon request. 
